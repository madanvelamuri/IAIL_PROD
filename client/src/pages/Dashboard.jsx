...
const Input = (props) => (
  <input
    {...props}
    className="border border-slate-300 px-4 py-2 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none"
  />
);
...
const Button = ({ children, color, ...props }) => {
  ...
  return (
    <button
      {...props}
      className={`${colors[color]} text-white px-6 py-2 rounded-xl shadow-md transition`}
    >
      {children}
    </button>
  );
